...
export default function AttendanceScreen() {
  ...
  function confirmDelete(record, driverName) {
    confirmAction("Delete attendance record?", `${driverName} — ${new Date(record.date).toLocaleDateString("en-IN")} — ${record.status}`, async () => {
      try {
        ...
        renderItem={({ item }) => {
          ...
          return (
            <View style={styles.recordRow}>
              ...
              <TouchableOpacity onPress={() => confirmDelete(item, driver?.fullName || "Driver")} style={styles.deleteBtn}>
...
const styles = StyleSheet.create({
  ...
  recordStatus: { fontWeight: "700", fontSize: 12, marginRight: 10 },
  deleteBtn: { padding: 4 },
