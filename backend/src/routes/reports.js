...
router.get("/export/attendance", requireAuth, requireRole("ADMIN"), async (req, res) => {
  ...
  await sendWorkbook(res, "attendance-report.xlsx", (workbook) => {
    ...
    records.forEach((r) => {
      sheet.addRow({
        ...
        empId: r.driver.employeeId,
        status: r.status,
        checkIn: r.checkIn ? r.checkIn.toLocaleString("en-IN") : "",
        checkOut: r.checkOut ? r.checkOut.toLocaleString("en-IN") : "",
        remarks: r.remarks || "",
      });
    });
    styleHeader(sheet);
