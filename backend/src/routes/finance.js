...
router.delete("/income/:id", requireAuth, requireRole("ADMIN"), async (req, res) => {
  const existing = await prisma.incomeTransaction.findUnique({ where: { id: req.params.id } });
...
router.delete("/expenses/:id", requireAuth, requireRole("ADMIN"), async (req, res) => {
  const existing = await prisma.expenseTransaction.findUnique({ where: { id: req.params.id } });
  if (!existing) return res.status(404).json({ error: "Not found" });
  if (existing.salaryPaymentId) {
    return res.status(409).json({ error: "This expense was auto-created by a salary payment and can't be deleted directly." });
  }
